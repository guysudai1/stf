  ...
  return createNewAdminUser().then(function(success) {
    ...
    return dbapi.getUsers().then(function(users) {
      return Promise.map(users, function(user) {
        if (user.privilege === apiutil.ROOT) {
          return true
        }
...
dbapi.getEmails = function() {
    ...
    return user('privilege').ne(apiutil.ADMIN).and(user('privilege').ne(apiutil.ROOT))
  })
  .getField('email'))
  .then(function(cursor) {
    return cursor.toArray()
  ...
  return dbapi.getGroup(id).then(function(group) {
    if (group) {
      ...
      return dbapi.loadUser(email).then(function(user) {
        if (user && apiutil.isPrivileged(user.privilege)) {
          return group
        }
        return false
      })
...
dbapi.updateUserSettings = function(email, changes) {
  var normalizedChanges = usersettings.normalizeChanges(changes)
  return db.run(r.table('users').get(email).update({
    settings: r.row('settings').default({}).merge(normalizedChanges)
  }))
}
