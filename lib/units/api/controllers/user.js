...
function addUserDevice(req, res) {
  ...
  var requestedTimeout
  if (req.hasOwnProperty('body')) {
    requestedTimeout = req.body.timeout || null
  }
  else {
    requestedTimeout = req.swagger.params.timeout.value || null
  }
  var timeout = usersettings.resolveTimeout(
    requestedTimeout
  , req.user.settings
  )
  const lock = {}
        ...
        messageListener = wirerouter()
          .on(wire.LeaveGroupMessage, function(channel, message) {
            if (message.serial === serial &&
                (message.owner.email === req.user.email ||
                 apiutil.isPrivileged(req.user.privilege))) {
              clearTimeout(responseTimer)
              req.options.channelRouter.removeListener(wireutil.global, messageListener)
