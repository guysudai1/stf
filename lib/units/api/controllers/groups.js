...
function addGroupDevices(req, res) {
  ...
  lockutil.lockGroup(req, res, lock).then(function(lockingSuccessed) {
    if (lockingSuccessed) {
      ...
      if (apiutil.isPrivileged(req.user.privilege) && req.user.email !== group.owner.email) {
...
function createGroup(req, res) {
  ...
  checkSchedule(res, null, _class, req.user.email, repetitions, req.user.privilege,
    ...
    // User privilege grants access to privileged operations; it does not
    // turn a user-owned group into the built-in root group.
