...
module.exports = function(options) {
  ...
  var totalsTimer, remoteAdbScanner
  var remoteAdbConfig = remoteAdb.validateConfig({
    subnet: options.remoteAdbSubnet
  , ports: options.remoteAdbPorts
  , intervalSeconds: options.remoteAdbScanInterval
  })

  if (remoteAdbConfig.enabled) {
    options.allowRemote = true
    options.remoteAdbSubnet = remoteAdbConfig.subnet.cidr
    options.remoteAdbPorts = remoteAdbConfig.ports
    options.remoteAdbScanInterval = remoteAdbConfig.intervalSeconds
    log.warn(
      'Remote ADB subnet discovery is enabled for %s; allowing remote devices ' +
      'implicitly (--allow-remote). Do not expose unauthenticated ADB on an untrusted network.'
    , options.remoteAdbSubnet
    )
  }
  ...
  client.trackDevices().then(function(tracker) {
    ...
    if (remoteAdbConfig.enabled) {
      ...
      remoteAdbScanner.start().catch(function(err) {
        log.error('Remote ADB scan failed: %s', err.message)
      })
    }
  })
